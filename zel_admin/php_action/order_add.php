...
if (isset($_POST['submitorder'])){
	if ($_SERVER["REQUEST_METHOD"] == "POST") {		
		$fr_id = test_input($_POST["fr_id"]);
		$order_date = test_input($_POST["txtdate"]);
		$sql1="SELECT order_id FROM orders WHERE fr_id='$fr_id' AND order_date='$order_date' AND order_status=1 ";
...
function order_add(){
	...
	$status=1;
	$currdate=date("Y-m-d");	
	$sql = "INSERT INTO orders (fr_id, order_date, order_status) VALUES ($fr_id, '$order_date', $status)";
