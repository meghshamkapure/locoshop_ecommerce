...
if (isset($_POST['register'])){
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
	 ...
	 $status=1;
	 if($ac_no==$conf_ac_no){
     $sql = "SELECT smob FROM seller WHERE smob='$smob' AND status=1 AND sid!=$sid";
	 $query = $conn->query($sql);
	 $count = $query->num_rows;
      if ($count >0){
		...
		$sql = "UPDATE seller SET sname='$sname', smob='$smob', spass='$spass', altmob='$altmob', gst='$gst', email='$email', pincode='$pincode', address='$address', bank_name='$bank_name', ac_name='$ac_name', ac_no='$ac_no', ifsc='$ifsc' WHERE sid=$sid";
	  ...
	  header( "Refresh:1; url=./edit_seller_profile.php?sid=$sid&error=$error&show=$show&alert=$alert", true, 303);
  }
  else
  {
...
<div class="container" style="margin-top:20px">
	<div class="row">
		...
		<div class="panel panel-info">
		    <div class="panel-heading" style="background-color:#004c91;color:white" align="center"> Update Seller Profile </div>
					...
					<div class="row">
						...
						<div class="form-group">
						  <label class="control-label">Name</label>
							<input class="form-control" type="hidden" id="sid" name="sid" value="<?php echo $sid;?>" placeholder="Seller Id" required>
						...
						<div class="form-group">
						  <label class="control-label">GST Number </label>
							<input class="form-control" type="number" id="gst" name= "gst" value="<?php echo $gst;?>" placeholder="GST Number" required>
						</div>						
						<div class="form-group">
