...
<form enctype="multipart/form-data" method="POST" action="php_action/order_edit.php" onsubmit="return true;">
	<div class="row">
		<div class="col-md-6">	
		<div class="form-group">
        <label class="control-label">Select Franchise User </label>
            <select class="form-control" id="fr_id" name="fr_id" required>
              <option value="">Select Franchise User</option>
			  <?php
				$query = "SELECT fr_id, name from franchise where fr_status=1 ORDER BY name ASC";
				$result = $conn->query($query);  
					while($row = $result->fetch_assoc()) {
					if($row['fr_id']==$fr_id){						
					echo "<option selected value='".$row['fr_id']."'>".$row['name']."</option>";
					}
					else{
					echo "<option value='".$row['fr_id']."'>".$row['name']."</option>";
					}
					}
				?>     
            </select>
